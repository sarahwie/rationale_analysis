for saliency in wrapper simple_gradient;
    ...
    for rationale in top_k max_length;
        ...
        SALIENCY=$saliency RATIONALE=$rationale RATIONALE_EXP_NAME=direct sbatch Cluster_scripts/multi_gpu_sbatch.sh bash Rationale_Analysis/commands/model_b_train_script.sh;
        done;
    done;