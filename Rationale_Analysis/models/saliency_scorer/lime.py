...
class LimeSaliency(SaliencyScorer):
    ...
    def score(self, metadata, **kwargs):
        ...
        def predict_proba(text_list):
            tokens_and_query = [[tokens[0]] + [Token(t) for t in selected_tokens.split(' ') if t != 'UNKWORDZ'] + query_tokens 
                                for selected_tokens in text_list]

            probs = []
            for i in range(0, len(tokens_and_query), self._batch_size) :
                document = self.regenerate_tokens(tokens_and_query[i:i+self._batch_size], metadata, device=self._keepsake_param.device)
                output = self._model['model'].forward(document=document, label=None, **kwargs)
                probs.append(output['probs'].cpu().data.numpy())
            return np.concatenate(probs, axis=0)

        explanation = self._explainer.explain_instance(" ".join(selection_tokens), predict_proba, num_features=num_features, labels=(predicted_label,))
