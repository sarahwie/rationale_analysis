...
if __name__ == '__main__':
	...
	if movies and multirc:
		...
		desired_inxs = './test_inxs_movies.txt'
	elif multirc:
		desired_inxs = './test_inxs_multirc.txt'
	...
	for line in open(infile, 'r'):
		...
		el_id = item['metadata']['annotation_id']
		if el_id in ids:
			docs.append(item)   
			found.add(el_id)
	...
	assert item['label'] == item['metadata']['label']

	# determine which format to use: 
	if movies:

		with open(outfile, 'w') as g:
			writer = csv.DictWriter(g, fieldnames=['annotation_id', 'text', 'pred_label', 'label', 'method'])
			writer.writeheader()
			for i, item in enumerate(docs):
				...
				rationale = ' '.join([i for i,z in zip(t, token_level_rationale) if z == 1])
				assert rationale in item['rationale']['document']

				# convert to dictionary & write to csv file
				writer.writerow({'text':rationale, 'annotation_id':item['metadata']['annotation_id'], 'pred_label':pred_label, 'label':true_label, 'method':os.path.splitext(os.path.basename(infile))[0]})

	elif multirc:

		with open(outfile, 'w') as g:
			writer = csv.DictWriter(g, fieldnames=['annotation_id', 'text', 'pred_label', 'label', 'method', 'question', 'answer'])
			writer.writeheader()
			for i, item in enumerate(docs):
				...
				# convert to dictionary & write to csv file
				writer.writerow({'text':rationale.encode("utf-8"), 'annotation_id':item['metadata']['annotation_id'], 'pred_label':pred_label, 'label':true_label,\
						'method':os.path.splitext(os.path.basename(infile))[0], 'question':item['metadata']['query'].split("||")[0].strip(), 'answer': item['metadata']['query'].split("||")[1].strip()})
